...
static int hello_probe(struct pci_dev *pdev, const struct pci_device_id *id)
{
 ...
 /* 补充: likely和unlikely用于提高cache的命中率,likely(a > b) 等价于(a > b),unlikely相反
  */
	if(unlikely(pci_request_regions(pdev,DEV_NAME))){   //申请I/O资源
...
static struct pci_driver hello_driver = {
    .name = DEV_NAME,
    .id_table = hello_ids,
    .probe = hello_probe,       //探测函数,主要为了探测设备,并做初始化,保存配置信息
    .remove = hello_remove,
};
...
static int hello_drv_init(void)
{
	...
	ret=alloc_chrdev_region(&my_device.devno,0,DEVICE_NUMBER,"hello");  //动态申请设备号
	if (ret < 0) {
