...
功能描述: 获取网易云在线音乐播放,目前未实现VIP歌曲和版权购买歌曲播放
作者: ShuJie
版本: Ver1.0
创建日期: 2022/2/9 19:43
...
class MusicApi:
    ...
    def GetMusicList(self, keywords):
        music_list = []
        tempUrl = 'http://music.163.com/song/media/outer/url?id='
        for offset in range(1):
            ...
            for music in result:
                ...
                if music['privilege']['dl'] == 0 and music['privilege']['pl'] == 0:
                    continue
                tempDict['song'] = music['name']
                tempDict['singer'] = music['ar'][0]['name']
                tempDict['songUrl'] = tempUrl + str(music['id']) + '.mp3'
                tempDict['picUrl'] = music['al']['picUrl']
                # 待续.... 后续看情况添加
                music_list.append(tempDict)
